...
create policy "menu_items: admin all"   on menu_items for all
  using (auth.jwt() ->> 'email' = any(string_to_array(current_setting('app.admin_emails', true), ',')));

-- Option groups/choices: public read
...
-- Orders: users see own orders; insert allowed for authenticated
...
create policy "polls: admin all"     on polls        for all
  using (auth.jwt() ->> 'email' = any(string_to_array(current_setting('app.admin_emails', true), ',')));
create policy "options: admin all"   on poll_options for all
  using (auth.jwt() ->> 'email' = any(string_to_array(current_setting('app.admin_emails', true), ',')));
